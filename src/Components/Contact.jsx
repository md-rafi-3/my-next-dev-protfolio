...
export default function ContactSection() {
  ...
  const sendEmail = (e) => {
    ...
    emailjs
      .sendForm(
        "service_sxyaf3i", // Service ID
        "template_df1x899", // Template ID
        form.current,
        "m5rJT5vqrxp5fZ4H2" // Public Key
      )
      .then(
        () => {
          setLoading(false);
  ...
  return (
    <section id="contact" className="relative py-20 px-6 md:px-16 overflow-hidden bg-[#0a0a0a]">
      ...
      <div className="grid md:grid-cols-2 gap-10 max-w-6xl mx-auto relative z-10">
        ...
        <div>
          ...
          <p className="text-gray-400 mb-6">
            I'm open to new projects, collaborations, and freelance work. Reach me via email or send a message below.
          </p>
          <ul className="space-y-4 text-gray-300">
            ...
            <li className="flex items-center gap-3">
              <FaWhatsapp className="text-xl text-[#ff00ff]" /> +8801618802042
            </li>
          </ul>
        </div>
        ...
        <div className="relative p-[2px] rounded-2xl bg-gradient-to-r from-[#ff00ff] via-[#00ffff] to-[#ff00ff] animate-border">
          <div className="bg-[#111111] p-6 rounded-2xl shadow-lg">
            ...
            <form ref={form} onSubmit={sendEmail} className="space-y-4">
              ...
              <div className="relative group" onMouseMove={handleMouseMove} onMouseLeave={() => setMousePos({ x: 0, y: 0 })}>
                <button
                  type="submit"
                  disabled={loading}
                  className={`relative z-10 w-full py-3 font-semibold rounded-full overflow-hidden transition-all duration-300 text-white ${loading ? "opacity-70 cursor-not-allowed" : ""}`}
                >
              ...
              {sent && <p className="text-center text-green-400 mt-3">✅ Message sent successfully!</p>}
            </form>
          </div>
        </div>
      </div>
      ...
      <style jsx>{`
        ...
        @keyframes slowFade { 0%,100% {opacity:0.7} 50% {opacity:1} }
        .animate-gradient { background-size:200% 200%; animation:gradient 6s ease infinite; }
        .animate-border { background-size:300% 300%; animation:shimmerBorder 8s linear infinite; }
        .animate-slowFade { animation:slowFade 10s ease-in-out infinite; }
      `}</style>
    </section>
  );
}
